...
################New############
def total_log_likelihood(theta, x, sigma2_1, model):
    """
    Compute the ordinary (unpenalized) total log-likelihood.

    This is the likelihood that should be reported in the results table
    and used to calculate AIC and BIC.
    """
    ...
    sigma2 = filter_sigma2(
        x, mu, lam, omega, alpha, beta, delta, gamma, sigma2_1
    )
...
def neg_log_likelihood(theta, x, sigma2_1, model):
    """
    Objective minimized during estimation.

    For GARCH-M-L, estimation uses the penalized log-likelihood:
        L_T(theta) - 0.001 * gamma^2

    The ordinary unpenalized likelihood is still used later
    for reporting LogLik, AIC and BIC.
    """
    ll = total_log_likelihood(theta, x, sigma2_1, model)

    if not np.isfinite(ll):
        return 1e10

    if model == "GARCH-M-L":
        gamma = unpack_params(theta, model)[6]
        ll -= 0.001 * gamma**2
...
# using Powell, which is more robust to the parameter penalties
            if not res.success:
                print(f"WARNING: {ticker} {model} BFGS did not converge: "
...
def get_theta_from_results(results, ticker, model):
    ...
    theta = np.array(
        [row[name] for name in PARAM_NAMES[model]],
        dtype=float
    )
...
def get_state_at_forecast_origin(
    ...
    x_history = history.values

    # Q4 parameter estimates: do NOT re-estimate here
    theta = get_theta_from_results(
        results,
        ticker,
        model,
    )
    ...
    if sigma2_history is None:
        raise ValueError(
            f"Invalid filtered variance path for {ticker} {model}."
        )
...
def update_sigma2_one_step(
    ...
    if not np.isfinite(sigma2_next) or sigma2_next <= 0:
        raise ValueError(
            "Invalid simulated variance encountered."
        )

    return sigma2_next


def simulate_next_return(
    sigma2_next,
    mu,
    lam,
    nu,
    ...
    x_next = (
        mu
        + lam * sigma2_next
        + np.sqrt(sigma2_next) * epsilon_next
    )

    return x_next, epsilon_next

### Step 3 
# Simulate one complete future return path


def simulate_return_path(
    state,
    model,
    horizon,
    rng,
    ...
    for h in range(horizon):
        ...
        x_next, epsilon_next = simulate_next_return(
            sigma2_next,
            mu,
            lam,
            nu,
...
### Step 4 
# Simulate many paths and construct compound returns
...
def compound_return(simulated_returns, horizon):
    ...
    return 100 * (
        np.prod(
            1 + simulated_returns[:horizon] / 100
        )
        - 1
    )
...
### Step 5: VaR
# Compute VaR from simulated compound returns
...
### Step 6
# Compute final VaR table for all stocks and models
...
print(
    var_results.round(2).to_string(index=False)
)

var_results.round(2).to_csv(
    "outputs/q6_var_results.csv",
    index=False,
)

# ---------- Question 6: AAPL validation for all three models ----------
